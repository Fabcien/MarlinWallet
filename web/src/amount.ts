...
export async function calculateTransactionAmountSats(wallet: Wallet, chronik: ChronikClient, txid: string): Promise<number> {
    try {
...
export function calculateMaxSpendableAmount(wallet: Wallet): number {    
    ...
    const inputs = spendableUtxos.map(utxo =>
        wallet.p2pkhUtxoToBuilderInput(utxo)!
    );
