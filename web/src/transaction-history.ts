...
export class TransactionHistoryManager {
    ...
    async loadTransactionHistory(chronik: any, address: string, reset: boolean = true): Promise<void> {
        ...
        try {
            ...
            const txHistoryResponse = await (this.chronik.address(this.address) as any).history(this.currentPage, 25);
